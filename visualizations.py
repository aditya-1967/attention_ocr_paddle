...
def visualize_attention(filename, output_dir, attentions, pred, accu, pad_width,
                        pad_height, threshold=1, normalize=False,
                        binarize=True, ground=None, flag=None):
    
    binary_data = filename[0]
    ...
    if len(result[0]) == 1:
        ...
        text1, text2, accu1, accu2 = result[0][0][1][0], result[0][1][1][0], result[0][0][1][1], result[0][1][1][1]
    ...
    with open(os.path.join(out_dir, 'word.txt'), 'w') as fword:
        fword.write(f'AOCR:    {pred}    {accu}' + '\n')
        fword.write(f'Padddle:    {text}   {score}' + '\n')
        if ground is not None:
            fword.write(f'Ground:    {ground}')
